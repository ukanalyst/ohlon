...
public class ReportingStats {
	...
	public String getBatchClassExecutionDetails(String identifier, String from, String to) {
		...
		try {
			...
			String sql = "SELECT MIN(PROCESS_ID) AS WORKFLOW_NAME, WORKFLOW_TYPE,  AVG(DURATION) AS AVGDURATION,  MIN(DURATION) AS MINDURATION,  MAX(DURATION) AS MAXDURATION,  AVG(1000.0 * TOTAL_NUMBER_DOCUMENTS / DURATION) AS AVGDOCPS,  MIN(1000.0 * TOTAL_NUMBER_DOCUMENTS / DURATION) AS MINDOCPS, MAX(1000.0 * TOTAL_NUMBER_DOCUMENTS / DURATION) AS MAXDOCPS,  AVG(1000.0 * TOTAL_NUMBER_PAGES / DURATION) AS AVGPAGEPS, MIN(1000.0 * TOTAL_NUMBER_PAGES / DURATION) AS MINPAGEPS, MAX(1000.0 * TOTAL_NUMBER_PAGES / DURATION) AS MAXPAGEPS, SUM(TOTAL_NUMBER_PAGES) AS NBOFPAGES, SUM(TOTAL_NUMBER_DOCUMENTS) AS NBOFDOCUMENTS, COUNT(DISTINCT PROCESS_KEY) AS NBOFBATCHINSTANCES FROM report_data WHERE DURATION > 0 AND BATCH_CLASS_ID = ?";
			...
			while (rs.next()) {
				...
				m.put("WORKFLOW_NAME", rs.getString("WORKFLOW_NAME").substring(0, rs.getString("WORKFLOW_NAME").lastIndexOf('-')));
